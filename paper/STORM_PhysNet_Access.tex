...
The matched control copies STORM's $d_{\mathrm{model}}$, layer count,
and head count only. It does not copy the feed-forward width:
PyTorch's default $\mathrm{dim\_feedforward}=2048$ is used, versus
$d_{\mathrm{ff}}=256$ in STORM. That is why the matched model has
$\approx 1.19\times 10^{6}$ parameters and STORM $\approx 3.43\times 10^{5}$.
...
Neither Transformer baseline uses STORM's residual-over-persistence
heads. The 1\,h PE$_{\mathrm{pers}}$ gap is consistent with that skip,
not only with the delay or gate.
...
number; for instance, the architecture-matched baseline remains near persistence at 1\,h (PE$_{\mathrm{pers}}\approx-0.01$), whereas STORM-Bz is clearly positive (PE$_{\mathrm{pers}}\approx+0.31$). Under synthetic input noise, STORM lost less 1\,h skill than the
Transformer in a held-out probe; we do not treat that probe as a
primary table. Horizon-restricted physics losses did not improve PE in a fifteen-seed
ablation.
