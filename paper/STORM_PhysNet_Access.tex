...
testing. No shuffling is applied.\footnote{The official tables use the released chronological cut
(63\,394 / 7\,924 / 15\,850 hours). The preprocessor default 70/15/15
cut is not the cut used for those numbers.}
...
\midrule
STORM-Bz bagged & \textbf{0.987} & \textbf{0.910} & \textbf{0.870} & 0.836 & --- \\
...
(\(0.809\) and \(0.807\)) and No-Gate slightly higher (\(0.819\)). Given overlapping intervals, we do not claim that
...
\(\mathrm{dim\_feedforward}=2048\)).
Neither baseline uses STORM's residual-over-persistence heads. An FFN-matched Transformer (\(d_{\mathrm{model}}=128\), two layers,
...
and updating only the forecast heads) raises 6\,h PE from 0.740
...
Among systems evaluated here, seed-bagged STORM-Bz reaches the strongest multi-horizon PE
(\(\mathrm{PE}_{6\mathrm{h}}=0.910\), \(\mathrm{PE}_{12\mathrm{h}}=0.870\));
the RDG/SDG single-model probes remain within seed noise of STORM-Bz.
...
externally, and its frozen outputs are provided in the repository.
